...
from icegraph.typing.common import ArrayG
...
class Stats(Processor[StatsConfig]):
    ...
    def _process(self, item: Envelope) -> Envelope | None:
        ...
        if len(splitmap) != len(main):
            raise RuntimeError(f"Length of splitmap does not match active table ({item.active}) row count.")
        ...
        # register stats
        item.set_local_attr("stats", payload)
        return item
