# Copyright (c) 2025 University of Maryland and the IceCube Collaboration.
# Developed by Taylor St Jean

from __future__ import annotations

from typing import ClassVar, Any

import torch
from torch import Tensor
import torch.nn.functional as F

from ..loss import LossFunction
from ..config import NLLLossConfig

__all__ = ["NLLLoss"]


class NLLLoss(LossFunction[NLLLossConfig]):
    name: ClassVar[str] = "nll"

    compatible = ("multiclass",)

    def build(self) -> None:
        return

    @classmethod
    def validate_config(cls, config: dict[str, Any]) -> NLLLossConfig:
        return NLLLossConfig(**config)

    def forward(self, out: Tensor, target: Tensor, /) -> Tensor:
        weight = torch.tensor(
            self.config.weight, device=out.device, dtype=torch.float32
        ) if self.config.weight is not None else None

        return F.cross_entropy(
            out, target,
            reduction=self.config.reduction,
            weight=weight,
            ignore_index=self.config.ignore_index
        )
