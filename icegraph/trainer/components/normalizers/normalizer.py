...
class Normalizer(Component[NormalizerContext]):
    name: str
    ...
    def __init__(self, params: Params) -> None:
        ...
        # transformer cache
        self._transforms: ModuleDict = ModuleDict({role: GroupTransform() for role in roles})

        # cache for scale and offset tensors
        # these must be registered buffers
        self._scale:    BufferedDict = BufferedDict.from_keys(roles)
        self._offset:   BufferedDict = BufferedDict.from_keys(roles)

    def on_attach(self, ctx: NormalizerContext) -> None:
        # load user config
        ...
        for role in ModelInputRole.all():
            # get stat service from context, only want training stats; we dont care about val or test stats
            stats = ctx.data.stats(Split.TRAIN, role)
