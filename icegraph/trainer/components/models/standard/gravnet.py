...
class GravNet(Model):
    ...
    def forward(self, x: torch.Tensor, batch: Optional[torch.LongTensor] = None) -> torch.Tensor:
        ...
        """
        error_msg = "Model has not been initialized (or only partially initialized), please call `init_model()`."
        assert self.blocks is not None, error_msg
        assert self.activation is not None, error_msg
        assert self.out is not None, error_msg
