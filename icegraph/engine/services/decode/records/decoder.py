...
class RecordDecoder(Plugin[C, RecordDecoderContext], ABC):
    """Decodes columnar record blocks, role by role.

    A decoder owns the record format: how to look a key up in a block and how
    that column's layout maps to decoded values. It returns columns — flat
    value tensors plus per-record row counts where rows vary — and has no
    concept of a batch; assembly is the dataset's job.
    """
    ...
    def _extract_features(self, block: RecordBlock, key: str) -> tuple[Tensor, ArrayI] | None:
        ...
        return torch.from_numpy(values), column.row_counts(block.height)
    ...
    @final
    def extract_auxiliary(self, block: RecordBlock, key: str) -> Float[Tensor, "B A_pre"] | Int[Tensor, "B A_pre"] | None:
    ...
    def _extract_edge_index(self, block: RecordBlock, key: str) -> tuple[Tensor, ArrayI] | None:
        ...
        return (
            ...
            column.row_counts(block.height),
        )
    ...
    def _fold_rows(column: Column, height: int, key: str) -> np.ndarray:
        ...
        if column.offsets is None:
            # already one row per record
            return values[:, None] if values.ndim == 1 else values

        counts = np.diff(column.offsets)
